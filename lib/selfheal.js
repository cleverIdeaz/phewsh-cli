...
function buildContextCore(cwd = process.cwd()) {
  ...
  try {
    ...
    output = sequence({
      target: 'claude-md',
      // Canonical native projections must preserve Project, current state,
      // accepted Next criteria, and constraints together. The standard 2k
      // pre-emission budget can let a long Vision + Next list crowd Current
      // Focus and constraints out entirely; the emitter still caps the
      // journal-shaped state sections, so the resulting block stays bounded.
      budget: 'canonical',
      sources: ['intent'],
      // decisions.md carries the Record — the fourth word. Without it the
      // canonical projection teaches Project and Next but drops what the last
      // session learned, which is the one thing a cold tool cannot reconstruct.
      sourceNames: ['vision.md', 'project.json', 'status.md', 'next.md', 'next.json', 'decisions.md'],
      excludeChunkSources: [
...
// next.md stayed days stale). Use Git ancestry rather than filesystem mtimes:
// a commit that updates code and intent together is current, while later
// code-only commits count as drift. Best-effort: 0 if not a git repo / no git.
const NARRATIVE_FILES = ['vision.md', 'plan.md', 'status.md', 'next.md'];
const INTENT_STATE_FILES = [
  ...NARRATIVE_FILES,
  'project.json',
  'next.json',
  'decisions.md',
];
const intentStatePaths = () => INTENT_STATE_FILES.map(file => path.join('.intent', file));
...
function commitsSinceIntent(cwd = process.cwd()) {
  try {
    ...
    const paths = intentStatePaths();
    const dirtyIntent = execFileSync('git', [
      'status', '--porcelain=v1', '--untracked-files=all', '--', ...paths,
    ], {
      cwd, encoding: 'utf-8', timeout: 2000, stdio: ['ignore', 'pipe', 'ignore'],
    });
    if (dirtyIntent.trim()) return 0;
    const anchor = execFileSync('git', ['log', '-1', '--format=%H', '--', ...paths], {
      cwd, encoding: 'utf-8', timeout: 2000, stdio: ['ignore', 'pipe', 'ignore'],
    }).trim();
    if (!anchor) return 0;
    const count = execFileSync('git', ['rev-list', '--count', `${anchor}..HEAD`], {
      cwd, encoding: 'utf-8', timeout: 2000, stdio: ['ignore', 'pipe', 'ignore'],
    }).trim();
...
function wrapDraft(cwd = process.cwd()) {
  try {
    ...
    const raw = (!dirtyIntent && anchor)
      ? execFileSync('git', ['log', `${anchor}..HEAD`, '--pretty=format:%s'], {
        cwd, encoding: 'utf-8', timeout: 2000, stdio: ['ignore', 'pipe', 'ignore'],
      })
      : '';
