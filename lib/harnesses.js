...
const HARNESSES = {
  'claude-code': { bin: 'claude',       label: 'Claude Code',  role: 'writes code',          bestFor: 'repo edits, tests, native coding loops',     auth: 'Claude subscription / Console', models: true, modelHints: ['fable', 'sonnet', 'opus', 'haiku'], streamFormat: 'claude-json', args: (p, m) => ['-p', p, '--output-format', 'stream-json', '--include-partial-messages', '--verbose', ...(m ? ['--model', m] : [])], interactiveArgs: (brief, m) => ['--append-system-prompt', brief, ...(m ? ['--model', m] : [])] },
  ...
  'kimi':        { bin: 'kimi',         label: 'Kimi Code',    role: 'codes & reviews',      bestFor: 'repo work, long-context coding, alternate framing', auth: 'Kimi membership / API key', models: true, args: (p, m) => ['-p', p, '--output-format', 'text', ...(m ? ['-m', m] : [])] },
...
const INSTALL = {
  ...
  'kimi':        'see kimi.com/code/docs/en/kimi-code-cli/ — install the official Kimi Code CLI',
...
/**
 * Runtime discovery + unlock metadata: which harnesses this machine can route
 * work through, and what it takes to enable one that is missing. Every surface
 * renders THIS rather than keeping its own list (`.intent/architecture.md`
 * §"Capability discovery"). Projected from facts the engine already holds.
 *
 * Scope boundary — this is NOT the full capability/action/receipt contract.
 * Cost, authority target, verification ceiling, and undo/rollback are part of
 * that contract and are not implemented by this slice. A surface must not
 * imply them from these fields.
 *
 * Field meanings, exactly:
 *   installed  — the binary is on PATH right now.
 *   headless   — can run a one-shot prompt with nobody at the keyboard.
 *                False means interactive-only (`args: null`).
 *   briefing   — the verified handoff brief can be auto-attached at
 *                interactive launch. False does NOT mean "not interactive":
 *                phewsh launches any installed harness interactively and falls
 *                back to putting the brief on the clipboard (commands/session.js).
 *   models     — accepts a model preference, passed straight through and
 *                validated by the harness. Never a model list phewsh owns.
 *
 * Deliberately absent: `bin`, `args`, `interactiveArgs`, `modelHints`. How to
 * invoke a harness is engine business, and a shell that learned it would drift.
 */
function harnessCapabilities() {
  return Object.entries(HARNESSES).map(([id, h]) => ({
    ...
    headless: !!h.args,
    briefing: !!h.interactiveArgs,
    models: !!h.models,
    install: INSTALL[id] || null,
    // Some hints are a runnable command, others are a docs pointer ("see
    // kiro.dev/downloads"). A surface that renders both as a shell command
    // tells people to run prose, so the engine — which owns the table — says
    // which kind it is instead of leaving a shell to guess from the string.
    installKind: INSTALL[id] ? (/^see /i.test(INSTALL[id]) ? 'docs' : 'command') : null,
  }));
}
