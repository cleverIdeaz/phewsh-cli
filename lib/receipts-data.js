...
function readRunReceipt(receiptId) {
  ...
  return {
    ...receipt,
    integrity,
    identityBinding: receipt.boundProjectRemote
      ? 'project-id+remote'
      : 'legacy-path-only',
  };
}
 ...
 */
function listRunReceipts({
  boundProjectId = null,
  boundProjectRemote = null,
  project = null,
  limit = 20,
} = {}) {
  return listDir(RUN_RECEIPTS_DIR)
    ...
    .filter((r) => r
      ...
      // When a current project asks for evidence, legacy path-only receipts and
      // receipts from a previous remote at that path stay out of the list.
      && (!boundProjectRemote || r.boundProjectRemote === boundProjectRemote)
...
function recordResultFile(record) {
  ensureDirs();
  const stem = safeFileStem(record.taskId);
  const body = JSON.stringify(record, null, 2);
  for (let attempt = 0; attempt < 8; attempt++) {
    const file = path.join(RESULTS_DIR, `${stem}_${Date.now()}_${crypto.randomUUID()}.json`);
    try {
  ...
  throw new Error('Could not allocate an unused result receipt filename');
}
