...
/**
 * A stable handle for a registered project, derived from its absolute path.
 *
 * Callers outside this machine (Ion, Desktop) must be able to name ONE exact
 * project without us publishing the path and without matching on a display
 * name — two repos can share a name, and "first match" is how the wrong
 * project gets opened. Deterministic, so the same repo keeps its id across
 * restarts, and one-way, so the id never leaks where the repo lives.
 */
function projectId(absPath) {
  return crypto.createHash('sha256').update(path.resolve(absPath)).digest('hex').slice(0, 16);
}

/** Resolve a registered project by its stable id. Never by name, never first-match. */
function findServeProjectById(id) {
  if (!id || typeof id !== 'string') return null;
  return serveProjects().find(p => projectId(p.path) === id) || null;
}
...
module.exports = { INDEX_FILE, SCAN_ROOTS, recordProject, listProjects, scanForProjects, scanForCandidates, fmtAgo, addServeProject, removeServeProject, serveProjects, projectId, findServeProjectById };
