...
function gitSnapshot(cwd = process.cwd(), packageJsonPath = null) {
  ...
  if (!head) return { available: false, head: null, tracked: [], untracked: [] };
  ...
  return { available: true, head, shortHead: head.slice(0, 8), latestCommitAt, committedPackageVersion, drift: statusDrift(cwd), ...status };
}
...
function loadClaimText(claims, cwd) {
  return claims.map(claim => {
    ...
    try { return { file: claim.file, text: fs.readFileSync(filePath, 'utf-8') }; }
...
function formatTruth(report) {
  ...
  lines.push(`Git HEAD: ${report.git.available ? `${report.git.shortHead} (${report.git.head})` : 'unknown (not a Git worktree)'}`);
