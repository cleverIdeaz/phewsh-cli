...
function currentNarrative(body) {
  return body.split(/^##\s+(?:Archive|Archived)\b.*$/im)[0].trim();
}
...
function parseStatus(filePath) {
  ...
  const lines = currentNarrative(body).split('\n');
...
// The Record. decisions.md is append-only and grows without bound, and a single
// entry can run to thousands of characters. Carry the most recent entries
// newest-first, each truncated to a headline, and let the section point at the
// file for the rest — the projection points at project truth, it does not become
// it. Both caps matter: without the per-entry cap, two long decisions consumed
// the whole pre-emission budget and evicted the accepted Next criteria.
const RECORD_ENTRIES = 3;
const RECORD_ENTRY_CHARS = 180;

function headline(entry) {
  if (entry.length <= RECORD_ENTRY_CHARS) return entry;
  const clipped = entry.slice(0, RECORD_ENTRY_CHARS);
  const lastSpace = clipped.lastIndexOf(' ');
  return `${clipped.slice(0, lastSpace > 0 ? lastSpace : RECORD_ENTRY_CHARS).trimEnd()}…`;
}
...
function parseDecisions(filePath) {
  ...
  // Entries are `- YYYY-MM-DD — text`, appended in chronological order.
  const entries = body
    .split('\n')
  ...
  const recent = entries.slice(-RECORD_ENTRIES).reverse().map(headline);

  return [{
    source: '.intent/decisions.md',
    sourceType: 'intent',
    kind: 'record',
    content: recent.join('\n'),
    timestamp,
    metadata: { frontmatter: meta, totalEntries: entries.length, shown: recent.length },
  }];
}
