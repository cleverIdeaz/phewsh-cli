...
function emit(chunks, options = {}) {
  ...
  // 7. Record — what happened and what we learned. The fourth word. Lean by
  // construction: the parser carries only the most recent entries, and this
  // section names the file so the next tool can read the rest itself.
  if (byKind.record?.length > 0) {
    sections.push('## Record');
    for (const chunk of byKind.record) {
      sections.push(chunk.content);
      const total = chunk.metadata?.totalEntries;
      const shown = chunk.metadata?.shown;
      if (total && shown && total > shown) {
        sections.push('');
        sections.push(`_Showing the ${shown} most recent of ${total} decisions — full record: \`.intent/decisions.md\`._`);
      } else {
        sections.push('');
        sections.push('_Full record: `.intent/decisions.md`._');
      }
    }
    sections.push('');
  }

  // 8. References — pointers to external systems
